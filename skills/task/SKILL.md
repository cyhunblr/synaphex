...
Call `synaphex_task_plan` with `project`, `slug`, `task`, `cwd`, `examiner_compact`, and optionally `reviewer_feedback` and `iteration`.
...
If **modify**: ask for feedback, then call `synaphex_task_plan` again with the feedback as `reviewer_feedback` and increment `iteration`. Present the new plan.
