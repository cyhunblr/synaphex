...
export type StandardAgentResultJsonSchema = Readonly<Record<string, unknown>>;

type JsonSchema = Record<string, unknown>;

/** Builds the provider-independent standard schema for a Core-shaped AgentResult. */
export class StandardAgentResultJsonSchemaBuilder {
  build(context: AgentContext): StandardAgentResultJsonSchema {
...
function reviewerResultSchema(context: AgentContext): JsonSchema {
  return resultObject(
    "reviewer",
    {
      ...
      report: configuredPayloadSchema(context.behavior?.outputFields ?? []),
    },
    ["reviewStatus", "report"],
    [
      {
      ...
      {
        if: {
          properties: { reviewStatus: { const: "PASS_WITH_WARNINGS" } },
          required: ["reviewStatus"],
        },
        then: {
          required: ["warnings"],
